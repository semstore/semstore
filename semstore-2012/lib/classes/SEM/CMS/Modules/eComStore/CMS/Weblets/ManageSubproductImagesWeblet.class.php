...
class ManageSubproductImagesWeblet extends Weblet
{
        /*
         *
	 * Class Constants
	 ...
	 */
        ...
        function ManageSubproductImagesWeblet ()
        {
                $numArgs = func_num_args();
                $args =& func_get_args();
                call_user_func_array(
                        array( &$this, '_ManageSubproductImagesWeblet'.$numArgs),
                        $args
                        );
        }
        
        
        function _ManageSubproductImagesWeblet0 ()
        {
                $this->_initialize();
        }
	 ...
	 */
        ...
        function _default ( &$httpRequest, &$httpResponse )
        {
                ...
                $template =& new CMSTemplate('modules/ecomstore/product_catalogue/subproduct_management/manage_subproduct_images.tpl');
                ...
                $subproductImages =& $subproduct->getImages();
                $template->assign('product_browser_thumbnail_subproduct_image', '');
                $template->assign('product_details_page_subproduct_image','');
                $template->assign('basket_subproduct_image','');
                $template->assign('subproduct_image','');
                
                if ( !is_null($subproductImages) )
                {
                        $template->assign('product_browser_thumbnail_subproduct_image',
                                $this->configurator->getParameter('product_images_webpath') . '/' .
                                $subproductImages->getBrowserThumbnailImage());
                        
                        $template->assign('product_details_page_subproduct_image',
                                $this->configurator->getParameter('product_images_webpath') . '/' .
                                $subproductImages->getProductDetailsPageImage());
                                
                        $template->assign('basket_subproduct_image',
                                $this->configurator->getParameter('product_images_webpath') . '/' .
                                $subproductImages->getBasketImage());
                        
                        $template->assign('subproduct_image',
                                $this->configurator->getParameter('product_images_webpath') . '/' .
                                $subproductImages->getProductImage());
                }
