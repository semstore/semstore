...
class ProductOrderTypeProductOrderAttributeGroup extends SEMObject
{
        /*
         *
	 * Class Constants
	 ...
	 */
        ...
        function ProductOrderTypeProductOrderAttributeGroup ()
        {
                $numArgs = func_num_args();
                $args =& func_get_args();
                call_user_func_array(
                        array(&$this, '_ProductOrderTypeProductOrderAttributeGroup'.$numArgs),
                        $args);
        }
        
        
        function _ProductOrderTypeProductOrderAttributeGroup0 ()
        {
                $this->_initialise();
                $do =& new ProductOrderTypeAttributeGroupDataObject();
	 ...
	 */
        ...
        function &find ( $criteria, &$connection )
        {
                ...
                {
                        $do =& $doArr[$index];
                        $attribute =& new ProductOrderTypeProductOrderAttributeGroup();
	 ...
	 */
        ...
        function remove ( $maintainIntegrity = TRUE )
        {
                ...
                $sql = 'UPDATE product_order_type_attribute_group ' .
                        'SET idx = idx - 1 ' .
                        'WHERE idx > ' . $idx;
        ...
        function &getAttributes ()
        {
                $sql = 'WHERE group_id = ' . $this->getId() .
                        ' ORDER BY idx';
                return ProductOrderTypeProductOrderAttribute::find(
        ...
        function addAttributeAtPosition ( &$newAttribute, $pos )
        {
                ...
                $sql = 'UPDATE product_order_type_attribute SET idx = idx + 1' .
                ...
                /* Create links in bridge table for all product orders to this
                 * attribute.
                 */
                
                $sql = sprintf('INSERT INTO %s (%s, %s) SELECT %s, %s FROM %s WHERE %s = %s',
                        'product_order_type_attribute_link',
                        'attribute_id',
                        'product_order_id',
                        $newAttribute->getId(),
                        'id',
                        'product_order',
                        'product_order.type_id',
                        $type->getId()
                        );
